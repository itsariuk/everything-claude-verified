#!/usr/bin/env node
/**
 * NanoClaw — Barebones Agent REPL for Everything Codex
 *
 * A persistent, session-aware AI agent loop that delegates to `codex exec`.
...
function askClaude(systemPrompt, history, userMessage) {
  ...
  if (result.status !== 0 && result.stderr) {
    return '[Error: codex exited with code ' + result.status + ': ' + result.stderr.trim() + ']';
  }
