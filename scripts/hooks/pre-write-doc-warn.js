 ...
 * expected directory (docs/, skills/, .codex/plans/), warns the user.
 *
...
process.stdin.on('end', () => {
  try {
    ...
    // Allow files in .codex/plans/, docs/, and skills/ directories
    const normalized = filePath.replace(/\\/g, '/');
    if (/\.codex\/plans\//.test(normalized) || /(^|\/)(docs|skills)\//.test(normalized)) {
      process.stdout.write(data);
      return;
    }
