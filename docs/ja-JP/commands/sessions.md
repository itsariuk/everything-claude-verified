...
Codexセッション履歴を管理 - `~/.codex/sessions/` に保存されたセッションのリスト表示、読み込み、エイリアス設定、編集を行います。
...
- セッションは `~/.codex/sessions/` にMarkdownファイルとして保存されます
- エイリアスは `~/.codex/session-aliases.json` に保存されます
