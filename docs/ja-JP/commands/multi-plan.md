...
- **計画のみ**: このコマンドはコンテキストの読み取りと`.codex/plan/*`計画ファイルへの書き込みを許可しますが、**本番コードを変更しない**
...
2. 計画を`.codex/plan/<feature-name>.md`に保存(要件から機能名を抽出、例: `user-auth`、`payment-module`)
3. **太字テキスト**でプロンプトを出力(**保存された実際のファイルパスを使用する必要があります**):

   ---
   **計画が生成され、`.codex/plan/actual-feature-name.md`に保存されました**
...
- **最初の計画**: `.codex/plan/<feature-name>.md`
- **反復バージョン**: `.codex/plan/<feature-name>-v2.md`、`.codex/plan/<feature-name>-v3.md`...
...
2. `.codex/plan/<feature-name>.md`ファイルを更新
3. 変更された計画を再提示
