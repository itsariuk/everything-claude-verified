...
These are upstream Claude Code behaviors, not ECC bugs. The entries below summarize the production-tested workarounds collected in [issue #644](https://github.com/itsariuk/everything-claude-verified/issues/644) on Claude Code `v2.1.79` (macOS, heavy hook usage, MCP connectors enabled). Treat them as pragmatic stopgaps until upstream fixes land.
...
- [issue #644](https://github.com/itsariuk/everything-claude-verified/issues/644) for the original report and tested environment.
