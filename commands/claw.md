---
description: Start the NanoClaw agent REPL — a persistent, session-aware AI assistant powered by the Codex CLI.
---
...
2. Loads conversation history from `~/.codex/claw/{session}.md`
3. Optionally loads ECC skill context from `CLAW_SKILLS` env var
4. Enters a blocking prompt loop — each user message is sent to `codex exec` with full history
5. Responses are appended to the session file for persistence across restarts

## Session Storage

Sessions are stored as Markdown files in `~/.codex/claw/`:

```
~/.codex/claw/default.md
~/.codex/claw/my-project.md
```
