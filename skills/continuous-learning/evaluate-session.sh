...
# Extracted skills saved to: ~/.codex/skills/learned/

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/config.json"
LEARNED_SKILLS_PATH="${HOME}/.codex/skills/learned"
...
if [ -f "$CONFIG_FILE" ]; then
  if ! command -v jq &>/dev/null; then
    ...
    LEARNED_SKILLS_PATH=$(jq -r '.learned_skills_path // "~/.codex/skills/learned/"' "$CONFIG_FILE" | sed "s|~|$HOME|")
  fi
fi
...
# Get transcript path from stdin JSON (Codex hook input)
