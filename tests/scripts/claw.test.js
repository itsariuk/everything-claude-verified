...
function runTests() {
  ...
  if (test('getClawDir() returns path ending in .codex/claw', () => {
    const dir = getClawDir();
    assert.ok(dir.endsWith(path.join('.codex', 'claw')),
      `Expected path ending in .codex/claw, got: ${dir}`);
  })) passed++; else failed++;

  if (test('getSessionPath("foo") returns correct .md path', () => {
    const p = getSessionPath('foo');
    assert.ok(p.endsWith(path.join('.codex', 'claw', 'foo.md')),
      `Expected path ending in .codex/claw/foo.md, got: ${p}`);
  })) passed++; else failed++;
